...
class ThirdPaginatedMenu(commands.Cog):
    ...
    async def paginated3(self, ctx, page: int = 0):
        ...
        menu.set_initial_page(page - 1)  # this takes a 0-based index, but users will likely use 1-based page numbers
        await menu.open()

    @commands.command()
    async def paginated4(self, ctx, page: str = None):
        """This example will show how to use strings instead of page numbers; for example, users can jump to
        a specific section of your help menu via the page mapping below. Try with `!paginated4 mod` to see it
        in action. There are other ways to design this, but a simple dictionary mapping is probably best for
        most use cases."""
